...
class Route {
  ...
  getHref(params = {}) {
    const { anchor, __format: format, ...qsParams } = params;

    return `${this.page}${format ? `.${format}` : ""}${encodeQuery(
      qsParams,
      true
    )}${anchor != null ? `#${anchor}` : ""}`;
  }
  ...
  getAs(params = {}) {
    ...
    const page = this.toPath(qsParams) || "/";

    for (const key in qsParams) {
      if (this.keyNames.includes(key)) delete qsParams[key];
    }

    return `${page}${format ? `.${format}` : ""}${encodeQuery(qsParams, true)}${
      anchor != null ? `#${anchor}` : ""
    }`;
  }
}
